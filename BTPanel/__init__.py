...
def config(action=None,pdata=None):
    ...
    defs = (
        ...
        "phpPeclUninstall","GetPeclInstallList","get_panel_asset","set_panel_asset","set_login_origin",
        "SetNoticeIgnore"
    )
...
def check_bind(pdata=None):
    ...
    import panelApi
    api_object = panelApi.panelApi()
    public.set_module_logs('app_check_bind', 'app_check_bind', 1)
...
def ws_mod_thread(_obj, get):
    ...
    if mod_result is None:
        return
    try:
        result = {'callback': get.ws_callback, 'result': mod_result}
        get._ws.send(public.getJson(result))
    except:
        return
